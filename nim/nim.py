...
class NimAI():
    ...
    def update_q_value(self, state, action, old_q, reward, future_rewards):
        ...
        if type(future_rewards) == int:
            new_val = reward + future_rewards
        else:
            new_val=reward
            for future_reward in future_rewards:
                new_val += future_reward

        self.q[tuple(state), tuple(action)] = old_q + self.alpha * (new_val - old_q)

        return old_q + self.alpha * (new_val - old_q)
    ...
    def actions(self, state):
        ...
        for rowNum in range(len(state)):
            row = state[rowNum]
            for box in range(1, row+1):
                actions.append((rowNum, box))
        return actions
    ...
    def choose_action(self, state, epsilon=True):
        ...
        """
        if epsilon and random.randint(1, 1/self.epsilon):
            return random.choice(self.actions(state))
        else:
            actions = []
            for action in self.actions(state):
                actions.append({'action': action, 'q': self.get_q_value(state, action)})
