        ...
        while True:
            ...
            if len(self.frontier)!=1:
                for node in self.frontier:
                    if abs(node.state[0]-self.end[0])+abs(node.state[1]-self.end[1])+node.num_explored<=abs(self.node.state[0]-self.end[0])+abs(self.node.state[1]-self.end[1])+self.node.num_explored:
                        self.node = node
